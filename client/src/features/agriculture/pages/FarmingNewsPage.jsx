        ...
        <div className="agri-page">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div className="agri-card" style={{ padding: '0.6rem 1.2rem', display: 'flex', gap: '1.5rem', alignItems: 'center', background: 'rgba(15, 23, 42, 0.4)', borderRadius: '16px' }}>
                            ...
                            {['all', 'news', 'announcement'].map(t => (
                                <button 
                                    ...
                                    style={{
                                        ...
                                        color: filter === t ? '#10b981' : 'rgba(255,255,255,0.4)',
                                        border: 'none',
                                        padding: '0.4rem 1rem',
                                        borderRadius: '8px',
                                        fontSize: '0.75rem',
            ...
            <div className="stats-grid" style={{ gridTemplateColumns: 'minmax(0, 1fr) 380px', gap: '3rem', alignItems: 'start' }}>
                        ...
                        <div className="agri-card" style={{ padding: 0, overflow: 'hidden', height: '480px', position: 'relative', display: 'flex', flexDirection: 'column' }}>
                            <div style={{ height: '100%', width: '100%', position: 'absolute', top: 0, left: 0 }}>
                                <img 
                                    ...
                                    alt="featured"
                                />
                                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '80%', background: 'linear-gradient(0deg, rgba(2,6,23,1) 0%, rgba(2,6,23,0.8) 40%, transparent 100%)' }}></div>
                            </div>
                            <div style={{ position: 'relative', zIndex: 2, padding: '3.5rem', marginTop: 'auto' }}>
                                <div style={{ background: 'rgba(16, 185, 129, 0.2)', color: '#10b981', padding: '0.5rem 1.2rem', borderRadius: '10px', fontSize: '0.75rem', fontWeight: 900, display: 'inline-block', marginBottom: '1.5rem', border: '1px solid rgba(16, 185, 129, 0.3)', letterSpacing: '1px' }}>FEATURED INTELLIGENCE</div>
                                <h2 style={{ fontSize: '2.8rem', fontWeight: 900, margin: '0 0 1.5rem 0', lineHeight: 1.1, letterSpacing: '-1px' }}>{latestNews[0].title}</h2>
                                <p style={{ fontSize: '1.1rem', opacity: 0.7, marginBottom: '2rem', maxWidth: '800px', lineHeight: 1.6, fontWeight: 500 }}>{latestNews[0].content?.substring(0, 200) || latestNews[0].description?.substring(0, 200)}...</p>
                                <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
                                    <button 
                                        onClick={() => window.open(latestNews[0].url || latestNews[0].link, '_blank')}
                                        style={{ background: '#fff', color: '#020617', padding: '1rem 2rem', borderRadius: '14px', border: 'none', fontWeight: 800, cursor: 'pointer', fontSize: '1rem' }}
                                    >
                                        READ FULL BRIEFING
                                    </button>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '2rem' }}>
                        {loading && (!news || news.length === 0) ? (
                            ...
                            <div 
                                key={idx} 
                                className="agri-card hover-bg" 
                                style={{ padding: '2rem', background: 'rgba(15, 23, 42, 0.4)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}
                            >
                                ...
                                <div>
                                    ...
                                    <h3 style={{ fontSize: '1.3rem', fontWeight: 800, margin: '0 0 1rem 0', lineHeight: 1.3, height: '3.4rem', overflow: 'hidden' }}>{item.title}</h3>
                                    <p style={{ fontSize: '0.95rem', opacity: 0.6, lineHeight: 1.6, height: '4.5rem', overflow: 'hidden', marginBottom: '1.5rem' }}>{item.content?.substring(0, 150) || item.description?.substring(0, 150)}...</p>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <button 
                                            onClick={() => window.open(item.url || item.link, '_blank')}
                                            style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: '0.6rem 1.2rem', borderRadius: '10px', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer' }}
                                        >
                                            CONTINUE READING
                                        </button>
                                        <button 
                                            onClick={() => handleSpeak(`${item.title}. ${item.content || item.description}`, idx)}
                                            style={{ color: speaking === idx ? 'var(--agri-green)' : 'rgba(255,255,255,0.3)', background: 'none', border: 'none', cursor: 'pointer' }}
                                        >
                                            <Volume2 size={20} />
                                        </button>
                                    </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', position: 'sticky', top: '100px' }}>
                    ...
                    <div className="agri-card" style={{ padding: '2.5rem', background: 'rgba(16, 185, 129, 0.05)', border: '1px solid rgba(16, 185, 129, 0.2)', borderRadius: '32px' }}>
                        <h4 style={{ fontSize: '1.2rem', fontWeight: 900, marginBottom: '2rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                            <div style={{width: '8px', height: '8px', background: '#10b981', borderRadius: '50%', boxShadow: '0 0 10px #10b981'}}></div>
                            Climate Telemetry
                        </h4>
                        {currentWeather ? (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div style={{ background: 'rgba(255,255,255,0.02)', padding: '1.8rem', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div>
                                            <div style={{ fontSize: '3.5rem', fontWeight: 900, letterSpacing: '-2px' }}>{Math.round(currentWeather.main?.temp)}°C</div>
                                            <div style={{ fontSize: '0.85rem', opacity: 0.5, fontWeight: 700, letterSpacing: '1px' }}>{(currentWeather.weather?.[0]?.description || 'Clear').toUpperCase()}</div>
                                        </div>
                                        <div style={{ textAlign: 'right' }}>
                                            <div style={{ fontSize: '1.2rem', fontWeight: 900, color: 'var(--agri-green)', marginBottom: '4px' }}>{currentWeather.main?.humidity}% RH</div>
                                            <div style={{ fontSize: '0.75rem', opacity: 0.4, fontWeight: 800 }}>HUMIDITY INDEX</div>
                                        </div>
                                    </div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                    <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '14px', textAlign: 'center' }}>
                                        <div style={{ fontSize: '1.1rem', fontWeight: 800 }}>{currentWeather.wind?.speed} m/s</div>
                                        <div style={{ fontSize: '0.65rem', opacity: 0.4, fontWeight: 700 }}>WIND VELOCITY</div>
                                    </div>
                                    <div style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '14px', textAlign: 'center' }}>
                                        <div style={{ fontSize: '1.1rem', fontWeight: 800 }}>{currentWeather.main?.pressure} hPa</div>
                                        <div style={{ fontSize: '0.65rem', opacity: 0.4, fontWeight: 700 }}>SURFACE PRESSURE</div>
                                    </div>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="agri-card" style={{ padding: '2.5rem', background: 'rgba(15, 23, 42, 0.4)', borderRadius: '32px' }}>
                        <h4 style={{ fontSize: '1.2rem', fontWeight: 900, marginBottom: '2.5rem', display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            {announcements.length > 0 ? announcements.map((ann, idx) => (
                                <div key={idx} style={{ paddingLeft: '1.5rem', borderLeft: '3px solid var(--agri-accent)', cursor: 'pointer' }} onClick={() => window.open(ann.link || ann.url, '_blank')}>
                                    <h5 style={{ fontSize: '1rem', fontWeight: 800, margin: '0 0 0.5rem 0', lineHeight: 1.4 }}>{ann.title}</h5>
